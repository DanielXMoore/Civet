...
2. **`.d.ts` neighbors for tsc** — `qwik build` runs `tsc --noEmit` separately from Vite, so it cannot resolve `.civet` imports on its own. The `build.civet` script (`civet --emit-declaration $(find src -name '*.civet')`) generates `<name>.civet.d.ts` next to each source before `build.types` runs tsc. Those `.d.ts` files are gitignored.
