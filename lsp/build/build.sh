...
if [ -n "${BUILD_COVERAGE-}" ]; then
  mkdir -p dist/coverage/lib
  cp node_modules/typescript/lib/lib.*.d.ts dist/coverage/lib
  cp source/tsconfig-lib.json dist/coverage/lib/tsconfig.json
  ...
  # (relative to dist/coverage/) which test-exclude rejects because they
  # start with "../". Absolute paths are made relative to cwd by test-exclude
  # and correctly matched against include: ["source"].
  MAP=dist/coverage/server.js.map
  node -e "
    const fs = require('fs')
    const path = require('path')
    const map = JSON.parse(fs.readFileSync('$MAP', 'utf8'))
    const base = path.resolve('dist/coverage')
