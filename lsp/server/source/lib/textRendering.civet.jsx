...
export function asPlainTextWithLinks(parts) {
  if (!parts) { return '' }
  if (typeof parts === 'string') { return parts }
  let out = ""
  let currentLink
      ...
      case "link":
        if (currentLink) {
          if (currentLink.target) {
            ...
            let ref;if ((ref = currentLink.text ?? currentLink.name)) {const text = ref;
              if (/^https?:/.test(text)) {
...
export function tagsToMarkdown(tags) {
  return tags.map(getTagDocumentation).join('  \n\n')
}

function getTagDocumentation(tag) {
  switch(tag.name) {
    case "augments":case "extends":case "param":case "template": {
    ...
    case "return":case "returns": {
      if (!tag.text?.length) { return undefined };break;
    }
  }
...
function getTagBody(tag) {
  if (tag.name === "template") {
    ...
    if (parts && typeof parts !== "string") {
      const params = parts
      .filter($ => $.kind === "typeParameterName")
      .map($1 => $1.text)
      .join(", ")

      const docs = parts
      .filter($2 => $2.kind === "text")
